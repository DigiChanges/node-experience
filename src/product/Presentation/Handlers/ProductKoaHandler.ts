...
import ProductRepPayload from '../../Domain/Payloads/IProductRepPayload';
import CriteriaPayload from '../../../Shared/Presentation/Validations/CriteriaPayload';
import IdPayload from '../../../Shared/Presentation/Requests/IdPayload';
import ProductUpdatePayload from '../../Domain/Payloads/IProductUpdatePayload';
...
ProductKoaHandler.post('/', AuthorizeKoaMiddleware(Permissions.PRODUCT_SAVE), async(ctx: DefaultContext) =>
{
    ...
    await responder.send(Product, ctx, config['HTTP_CREATED'], new DefaultMessageTransformer(ResponseMessageEnum.CREATED));
});
...
ProductKoaHandler.put('/:id', AuthorizeKoaMiddleware(Permissions.PRODUCT_UPDATE), async(ctx: DefaultContext) =>
{
    ...
    await responder.send(Product, ctx, config['HTTP_CREATED'], new DefaultMessageTransformer(ResponseMessageEnum.UPDATED));
});
...
ProductKoaHandler.delete('/:id', AuthorizeKoaMiddleware(Permissions.PRODUCT_DELETE), async(ctx: DefaultContext) =>
{
    ...
    await responder.send(Product, ctx, config['HTTP_CREATED'], new ProductTransformer());
});
