    ...
    describe('Item Fails', () =>
    {
        ...
        test('Add Item /items', async done => {
           const payload = {
            ...
            expect(message).toBe('Failed Request.');

            expect(error.property).toBe('type');
            expect(error.constraints.isInt).toBe('type must be an integer number');
        ...
        test('Get Item /items/:id', async done => {
            ...
            expect(error.constraints.isUuid).toBe('id must be an UUID');
        ...
        test('Update Item /items/:id', async done => {
            ...
            expect(errorName.property).toBe('name');
            expect(errorName.constraints.isString).toBeDefined();
            expect(errorName.constraints.isString).toBe('name must be a string');

            expect(errorType.property).toBe('type');
            expect(errorType.constraints.isInt).toBeDefined();
            expect(errorType.constraints.isInt).toBe('type must be an integer number');
        ...
        test('Delete Item error /items/:id', async done => {
            ...
            expect(deleteErrorResponse.statusCode).toBe(400);
            expect(status).toBe('error');
            expect(statusCode).toBe('HTTP_BAD_REQUEST');
